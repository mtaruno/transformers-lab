...
class Engine:
    ...
    def __init__(self):
        ...
        self.kv_cache = {}
        self.seq_pos = 0  # Track sequence position for causal masking
    ...
    def generate(self, input_string, rounds=20):
        ...
        # Process all input tokens with prefill=True to initialize KV cache
        new_token = self.run(output_ids, prefill=True)
        output_ids.append(new_token)

        for round in range(rounds - 1):
            # print(f"Round {round}")
            # Process only the latest token with prefill=False to use the KV cache
            new_token = self.run([new_token], prefill=False)
            output_ids.append(new_token)
...
if __name__ == "__main__":
    ...
    with open("single_batch_times.csv", "w") as f:
        writer = csv.writer(f)
